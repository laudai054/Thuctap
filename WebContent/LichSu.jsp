<%@page import="bean.LichSuDatHangbean"%>
...
<body>
 ...
 %>
	...
	<%
	ArrayList<LichSuDatHangbean> dsLSDH = (ArrayList<LichSuDatHangbean>) session.getAttribute("dsLSDH");
	...
	<div class="container">
	<br>
		<h2 align="center" style="color:white;"><b>LỊCH SỬ MUA HÀNG</b></h2><br>

		<table >
			<tr align="center">
				<td valign="top" style="width: 1050px">
					<table class="table table-bordered" style="width: 1000px; background-color: white">
						<%
						if (session.getAttribute("dsLSDH") != null && !dsLSDH.isEmpty()) { //checknull
						%>

						<%
						LichSuDatHangbean ls1 = dsLSDH.get(0);
						%>
						<p style="color: blue">
							<span>Mã Khách hàng: </span><%=ls1.getMakh()%></p>
						<%
						for (LichSuDatHangbean ls : dsLSDH) {
						%>
						<tr style="border: 1px solid black;">
							<td>
								<p>
									<span style="color: red">Tên Giày: </span><%=ls.getTenSach()%></p>
								<p>
									<span style="color: red">Giá: </span><%=ls.getGia()%>
									VNĐ
								</p>
								<p>
									<span style="color: red">Số lượng mua: </span><%=ls.getSoluongMua()%></p>
								<p>
									<span style="color: red">Thành tiền: </span><%=ls.getThanhTien()%>
									VNĐ
								</p>
								<p style="font-weight: bold">
									<span style="color: red">Trạng thái: </span><%=ls.isDamua() ? "Đã mua" : "Chưa mua"%></p>
							</td>
						</tr>
						<%
						}
